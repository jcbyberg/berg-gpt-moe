...
async def chat_completions(req: ChatCompletionRequest, request: Request):
    ...
    if req.stream:
        return StreamingResponse(
            stream_hive_mind_response(
                query, system_prompt, agent_registry, hot_memory, request_id, created_time
            ),
    ...
    try:
        ...
        selected_agents = [
            agent_registry.get_agent(aid) for aid in planned_ids if agent_registry.get_agent(aid)
        ]
        ...
        if successful_results:
            ...
            final_answer = (
                "I apologize, but my agent swarm failed to retrieve the necessary information."
            )
...
async def stream_hive_mind_response(
    ...
    try:
        ...
        # 3. Synthesis
        yield f"data: {create_chunk('Available data collected. 🧪 *Synthesizing final answer...*')}\n\n"
