...
class HotMemoryManager:
    ...
    async def initialize(self) -> None:
        ...
        schema = IndexSchema.from_dict(
            {
                "index": {"name": "hive_hot_memory", "prefix": "ctx:", "storage_type": "hash"},
                "fields": [
    ...
    async def read(
        self, agent_id: str, limit: int = 10, data_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def semantic_search(
        self, query_vector: List[float], num_results: int = 5, agent_filter: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        if not self._redis_client:
            return {"total_keys": 0, "index_info": {}}

        return {"total_keys": self._redis_client.dbsize(), "index_info": self.index.info()}

    async def get_by_prefix(self, prefix: str, limit: int = 100) -> List[Dict[str, Any]]:
