...
class MCPAgent(ABC):
    ...
    async def call_tool(
        self, tool_name: str, args: Dict[str, Any], write_to_memory: bool = True
    ) -> Dict[str, Any]:
    ...
    async def _simulate_tool_call(self, tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if "search" in tool_name.lower():
            return {
                "results": [
                    {
                        "title": f"Result for {args.get('query', 'unknown')} from {self.config.name}",
                        "url": "https://example.com",
                    },
                    {
                        "title": f"Another result for {args.get('query', 'unknown')}",
                        "url": "https://example.com",
                    },
                ],
            ...
            return {"message": f"Executed {tool_name}", "status": "success", "data": args}

    async def _write_to_memory(self, tool_name: str, args: Dict[str, Any], result: Any) -> None:
