    ...
    async def process_task(self, task: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for root, dirs, files in os.walk(self.root_dir):
                for file in files:
                    if task.lower() in file.lower() or any(
                        ext in task for ext in [".py", ".json", ".md", ".txt"]
                    ):
            ...
            result_data = {"files_found": found_files, "root": self.root_dir}

            await self._write_to_memory(
                tool_name="filesystem_search", args={"query": task}, result=result_data
            )
