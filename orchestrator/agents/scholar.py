...
class TheScholarAgent(MCPAgent):
    ...
    async def process_task(self, task: str) -> Dict[str, Any]:
        ...
        try:
            ...
            search = arxiv.Search(query=task, max_results=5, sort_by=arxiv.SortCriterion.Relevance)
            ...
            summary_text = "\n".join(
                [f"- {p['title']} ({p['published'][:4]}): {p['summary'][:150]}..." for p in papers]
            )

            # Write to memory
            await self._write_to_memory(
                tool_name="arxiv_search", args={"query": task}, result=papers
            )
