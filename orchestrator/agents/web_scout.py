        ...
        try:
            ...
            response = await loop.run_in_executor(
                ...
                lambda: self.tavily.search(
                    query=task, search_depth="advanced", max_results=5, include_raw_content=True
                ),
            )

            summary = "\n".join(
                [f"- {r['title']}: {r['content'][:200]}..." for r in response.get("results", [])]
            )

            # Write to memory
            await self._write_to_memory(
                tool_name="tavily_search", args={"query": task}, result=response
            )
