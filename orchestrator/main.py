...
from orchestrator.ai import get_ai_client, gemini_client, zai_client
...
async def root():
    ...
    return {
        ...
        "version": "2.1.0",
        "status": "online",
        "ai_enabled": zai_client._configured or gemini_client._configured,
    }
...
async def health_check():
    ...
    return {
        ...
        "services": {
            ...
            "zai": zai_client._configured,
            "gemini": gemini_client._configured,
        },
    }
...
async def dispatch_mission(query_request: Dict[str, Any]):
    ...
    if force_agents:
        ...
        selected_agents = [
            agent_registry.get_agent(aid) for aid in force_agents if agent_registry.get_agent(aid)
        ]
    else:
        # AI Planning
        try:
            ...
            planned_ids = await zai_client.plan_mission(query, available_agents)
    ...
    if successful_results:
        try:
            final_answer = await zai_client.synthesize_results(query, successful_results)
        except Exception as e:
...
async def startup_event():
    ...
    try:
        ...
        # Initialize AI
        zai_client.configure()
        gemini_client.configure()
        logger.info("AI Providers - ZAI: configured, Gemini: configured")
